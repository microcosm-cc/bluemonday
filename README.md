...
* Investigate whether devs want an API for Sanitize() that accepts a []byte and returns a []byte, to make it simpler to chain it onto [blackfriday](https://github.com/russross/blackfriday), i.e. `bluemonday.UGCPolicy().Sanitize(blackfriday.MarkdownCommon(input))`
